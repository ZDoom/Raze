...
//	memset(&currentpacketnumber, 0, sizeof(unsigned long) * MAXPLAYERS);
    ...
    gcom = init_network_transport(argv, i+1);
    if (gcom == NULL)
    {
...
static int connect_to_everyone(gcomtype *gcom, int myip, int bcast)
{
    ...
    Bmemset(heard_from, '\0', sizeof(heard_from));
    ...
    while ((remaining) && (!quitevent))
    {
        ...
        if ((rc > 0) && (ip) && ((ip != myip) || (port != udpport)))
        {
            ...
            {
                packet.id = B_LITTLE16(packet.id);
    ...
    Bmemmove(&allowed_addresses[1], &allowed_addresses[0],
...
gcomtype *init_network_transport(char **ARGV, int argpos)
{
    ...
    if (retval != NULL)
    {
        ...
        Bmemset(retval, '\0', sizeof(gcomtype));
        Bmemset(allowed_addresses, '\0', sizeof(allowed_addresses));
        udpsocket = -1;
        udpport = BUILD_DEFAULT_UDP_PORT;
        udpmode = udpmode_peer;
