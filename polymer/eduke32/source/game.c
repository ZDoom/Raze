...
/*
    if (numplayers > 1)
    {
        ...
        for (j=MAXPLAYERS-1; j>=0; j--)
        {
            ...
            if (net_client)
                enet_peer_send(net_peer, 0, enet_packet_create(packbuf, 1, ENET_PACKET_FLAG_RELIABLE));
            else if (net_server)
                enet_host_broadcast(net_server, 0, enet_packet_create(packbuf, 1, ENET_PACKET_FLAG_RELIABLE));
        }
    }
*/
