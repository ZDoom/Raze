...
int movesprite_ex_r(DDukeActor* actor, int xchange, int ychange, int zchange, unsigned int cliptype, Collision &result)
{
	...
	if (bg)
	{
		...
		if ((result.type == kHitSector || result.type == kHitSprite) && (actor->cgg == 0)) actor->spr.ang += 768;
	}
	else
	{
	...
	while (auto act = it.Next())
	{
		...
		switch(act->spr.picnum)
		{
			...
			case EMPTYBIKE:
				...
				if (sectp->lotag == 1)
				{
					SetActor(act, { act->spr.x,act->spr.y,act->floorz + (16 << 8) });
				}
				break;
	...
	if (t[4] > 0)
	{
		...
		if (t[4] < ((k >> 1) - (k >> 3)))
		{
			...
			if ((!isRRRA() || lastlevel) && hulkspawn)
			{
				...
				if (!hulkspawn)
				{
					ns = EGS(actor->spr.sector(), actor->spr.x, actor->spr.y, actor->spr.sector()->ceilingz + 119428, 3677, -8, 16, 16, 0, 0, 0, actor, 5);
					if (ns)
					{
