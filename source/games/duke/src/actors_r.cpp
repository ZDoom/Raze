...
CLEAR_THE_BOLT:
	...
	if (actor->spr.picnum == (BOLT1 + 1) && (krand() & 1) && (tileflags(sectp->floorpicnum) & TFLAG_ELECTRIC))
	...
	if (chickenplant)
	{
		...
		while (auto act = it.Next())
		{
			...
			case CHICKENASPAWN:
				...
				if (act->spr.lotag < 0)
				{
					spawn(act, CHICKENA)->spr.ang = act->spr.ang;
					act->spr.lotag = 128;
				}
				break;
			case CHICKENCSPAWN:
				...
				if (act->spr.lotag < 0)
				{
					spawn(act, CHICKENC)->spr.ang = act->spr.ang;
					act->spr.lotag = 256;
				}
				break;
			case FEATHERSPAWN:
			...
			case LOAFSPAWN:
				...
				if (act->spr.lotag < 0)
				{
					spawn(act, CHICKENLOAF)->spr.ang = act->spr.ang;
					act->spr.lotag = 448;
				}
				break;
			case NUGGETSPAWN:
				...
				if (act->spr.lotag < 0)
				{
					spawn(act, CHICKENNUGGET)->spr.ang = act->spr.ang;
					act->spr.lotag = 64;
				}
				break;
			case ROASTSPAWN:
				...
				if (act->spr.lotag < 0)
				{
					spawn(act, ROASTEDCHICKEN)->spr.ang = act->spr.ang;
					act->spr.lotag = 512;
				}
				break;
			case BONELESSSPAWN:
				...
				if (act->spr.lotag < 0)
				{
					spawn(act, BONELESSCHICKEN)->spr.ang = act->spr.ang;
					act->spr.lotag = 224;
				}
				break;
			case JIBSSPAWN:
