...
void QuakeViewChange(PLAYERp pp, int *z_diff, int *x_diff, int *y_diff, short *ang_diff)
{
    ...
    *ang_diff = 0;
	...
	while (auto actor = it.Next())
	{
        ...
        dist = FindDistance3D(pp->posx - sp->pos.X, pp->posy - sp->y, pp->posz - sp->z);
...
int SetExpQuake(DSWActor* actor)
{
    ...
    SpawnQuake(sp->sector(), sp->pos.X, sp->y, sp->z,  40, 4, 20000); // !JIM! was 8, 40000
    return 0;
}
...
int SetGunQuake(DSWActor* actor)
{
    ...
    SpawnQuake(sp->sector(), sp->pos.X, sp->y, sp->z,  40, 8, 40000);
...
int SetNuclearQuake(DSWActor* actor)
{
    ...
    SpawnQuake(sp->sector(), sp->pos.X, sp->y, sp->z, 400, 8, 64000);
    return 0;
}
...
int SetSumoQuake(DSWActor* actor)
{
    ...
    SpawnQuake(sp->sector(), sp->pos.X, sp->y, sp->z,  120, 4, 20000);
    return 0;
}
...
int SetSumoFartQuake(DSWActor* actor)
{
    ...
    SpawnQuake(sp->sector(), sp->pos.X, sp->y, sp->z,  60, 4, 4000);
    return 0;
}
